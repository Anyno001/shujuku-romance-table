export const noDiceRomanceSpecialV73TwoPlayer = {
  "id": "no-dice-romance-special-v7-3-two-player",
  "title": "无骰子-恋爱特化表v7.3（双人成行适配）",
  "databaseKey": "no-dice-romance-special-v7-3-two-player",
  "template": {
    "mate": {
      "type": "chatSheets",
      "version": 2,
    ...
    "sheet_romance_targets": {
      ...
      "sourceData": {
        ...
        "insertNode": "新恋爱对象登场时新增一行。\n\n【格式约束】\nrelation_state 必须写\"角色名:关系标签\"；同一角色多标签用 , ；不同角色用 ; 。\n\nSQL示例: INSERT INTO romance_targets (row_id, name, gender, age, brief_intro, appearance, outfit_text, location_name, presence_status, relation_state, current_thought, past_experience, intimacy_count)\nVALUES ((SELECT COALESCE(MAX(row_id), 0) + 1 FROM romance_targets), '神谷真', '男', '32', '地下情报贩子，经营旧港仓区仓库', '身高180cm，黑发，左眼有一道斜长刀疤', '黑色修身西装，白衬衫，黑色皮鞋', '伪装:74; 情报收集:88', '旧港仓区三号仓库', '离场', '主角:认识,若即若离', '这个人有点意思……先观察看看，不要靠太近', '长期向多方出售消息，见过太多黑暗，从不轻易相信任何人', 0);",
    ...
    "sheet_important_non_romance": {
      ...
      "sourceData": {
        ...
        "insertNode": "新非恋爱重要角色登场时新增。\n\n【格式约束】\nrelation_text 必系描述\"；同一角色多项描述用英文逗号 , 分隔；不同角色之间用英文分号 ; 分隔。至少包含与主角的关系。\n\nSQL示例: INSERT INTO important_non_romance (row_id, name, gender, age, brief_intro, appearance, outfit_text, location_name, presence_status, relation_text, past_experience) VALUES ((SELECT COALESCE(MAX(row_id), 0) + 1 FROM important_non_romance), '黑市商人', '男', 45, '经营城南杂货铺', '左眼带有刀疤', '深灰立领风衣配黑皮手套', '城南杂货铺', '在场', '主角:曾救过其性命,长期供货人', '长期在地下黑市进行交易。');",
    ...
    "sheet_outfits": {
      ...
      "sourceData": {
        ...
        "initNode": "按设定添加主角初始重要装扮，禁止把主角所有衣服都初始化记录。\n\n只初始化：\n1.开场正在穿且需要持续引用的装扮；\n2.固定常穿、具有识别度的装扮；\n3.与开场剧情、约会、身份、职业或关系有关的装扮。\n\nSQL示例: INSERT INTO outfits (row_id, owner_role, outfit_name, outfit_type, current_status, appearance_desc, obtained_from, scene_meaning) VALUES ((SELECT COALESCE(MAX(row_id), 0) + 1 FROM outfits), '主角', '白色连衣裙', '约会装', '收纳中', '及膝白裙，裙摆有细蕾丝', '赠送', '约会时对方赠送的，适用于纪念日');",
        "deleteNode": "禁止。",
        ...
        "insertNode": "主角获得、穿上或展示新的重要装扮时新增。\n\n新增条件：\n1.当前正在穿且正文后续可能继续引用的装扮；\n2.重要约会、告白、和解、争执、纪念日中的装扮；\n3.恋爱对象赠送、挑选、评价、借用或共同购买的装扮；\n4.主角特意为某次见面准备的装扮；\n5.后续会反复出现的识别性装扮。\n\n禁止新增：\n1.普通日常衣物；\n2.只出现一次、无后续价值的背景衣服；\n3.拆分过细的单件衣物；\n4.与装扮无关的普通物品。\n\nSQL示例: INSERT INTO outfits (row_id, owner_role, outfit_name, outfit_type, current_status, appearance_desc, obtained_from, scene_meaning) VALUES ((SELECT COALESCE(MAX(row_id), 0) + 1 FROM outfits), '主角', '浅蓝发带', '饰品', '正在穿', '浅蓝色细发带，颜色很干净', '赠送', '洛斯莉赠送的，适合约会时佩戴');",
    ...
    "sheet_bwxtt33d5": {
      ...
      "name": "选项表",
      "sourceData": {
        "note": "记录每轮剧情后提供给玩家的5条行动选项，此表固定保留5行，每轮必须覆盖写入全部5行。\n\n- 列1: 选项一 option_1\n- 列2: 选项二 option_2\n- 列3: 选项三 option_3\n- 列4: 选项四 option_4\n- 列5: 选项五 option_5\n\n【展示文本约束】\n1. 数量：必须恰好 5 条，每条非空。\n2. 视角：默认由玩家角色发起行动，写\"玩家角色对角色做什么\"。不写主角内心独白，不写角色视角的心理活动。\n3. 写法：使用限知第三人称或省略主语的外部动作句，只描写外部可见的行动。禁止使用第一人称、第二人称，禁止称呼用户姓名。\n   √ 小心翼翼试探她的反应\n   √ 假装没注意到他的目光，继续往前走\n   √ 在她开口之前，把一直没送出手的信放在桌上\n   × 你看着他，心里想他是不是也喜欢我\n   × 艾莉丝内心一阵甜蜜，却装作若无其事\n\n4. 内容方向：\n5 条选项必须紧贴最新剧情中的恋爱互动、误会、靠近、退缩、吃醋、邀约、告白、和解、等待回应等关系节点。\n每条都要让读者看出会推动关系进展、制造误会、拉近距离或引发转折。\n\n5. 五条选项方向必须区分：\n- 第 1 条：主动推进 —— 主动做出一个贴合当前剧情的行动，使关系或事件继续向前。\n- 第 2 条：正面回应 —— 对当前的冲突、暗示、请求、沉默或变化给出明确反应。\n- 第 3 条：保留/回避 —— 选择克制、观察、转移、沉默、退后或掩饰，保留余地但制造张力。\n- 第 4 条：制造变量 —— 借当前已有的人、物、环境、秘密、承诺或突发变化改变局面。\n- 第 5 条：快进剧情 —— 跳出当前即时互动，推进到下一个关键时间点、场景或事件节点；不得写成纯功能按钮，不得直接替玩家决定最终结果。\n\n【生成要求】\n1. 每轮必须生成并覆盖写入恰好5条行动选项。\n2. row_id 必须固定为1、2、3、4、5。\n3. display_text 必须是非空字符串。\n4. 五条选项必须紧贴最新剧情，优先围绕当前恋爱互动、情感波动、关系阻碍、约定、误会、试探、邀约、告白、吃醋、和解等内容。\n5. 若当前剧情无明显恋爱节点，则围绕当前场景中最可能推动关系或剧情变化的行动生成。\n6. 不得生成 row_id=1~5 之外的记录。\n7. 不得少写、漏写、复用旧选项或生成空选项。",
        "initNode": "首次生成行动选项时，根据初始剧情生成5条行动选项，优先围绕当前恋爱互动、情感波动、关系阻碍、约定、误会、试探、邀约、告白、吃醋、和解等内容。若初始剧情无明显恋爱节点，则围绕当前场景中最可能推动关系或剧情变化的行动生成。display_text 必须是非空字符串，禁止照搬示例。",
        "insertNode": "禁止。",
        "updateNode": "每轮交互后必须根据最新剧情重新生成5条行动选项，新选项必须紧密贴合本轮发生的恋爱互动、情绪转折、未决事件、新产生的约定或误会。展示文本必须遵循外部动作句规则，不写内心独白，不使用第一人称或第二人称。\n\nSQL示例: UPDATE options SET option_1 = '新选项一', option_2 = '新选项二', option_3 = '新选项三', option_4 = '新选项四', option_5 = '新选项五' WHERE row_id = 1;",
        "deleteNode": "禁止。",
        "ddl": "CREATE TABLE options ( -- 选项表\n  row_id INTEGER PRIMARY KEY, -- 行号\n  option_1 TEXT NOT NULL, -- 选项一\n  option_2 TEXT NOT NULL, -- 选项二\n  option_3 TEXT NOT NULL, -- 选项三\n  option_4 TEXT NOT NULL, -- 选项四\n  option_5 TEXT NOT NULL -- 选项五\n);"
      },
      "content": [
        [
          "row_id",
